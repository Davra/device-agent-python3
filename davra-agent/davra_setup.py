...
def setDeviceMqttBrokerSecurity():
    if("mqttRestrictions" in comDavra.conf and "localhost" in comDavra.conf["mqttRestrictions"]):
        comDavra.runCommandWithTimeout('sudo echo "listener 1883 localhost\nallow_anonymous true" > /etc/mosquitto/conf.d/localhost.conf', 5)
...
def configGetServer():
    if('server' not in comDavra.conf):
        ...
        userInput = input("Server location? (eg http://myName.davra.com) : ")
...
def configGetApiTokenOfDevice():
    ...
    if('apiToken' not in comDavra.conf):
        ...
        if(userInput == ""):
            ...
            userInput = input("API Token of this device? : ")
    else:
